...
Rejection sampling can be used to draw samples from a target density $p(\theta | y)$ or its unnormalized version $q(\theta | y)$.
...
-   If $p(\theta = \theta^{\ast| y) > 0$, then $g(\theta = \theta^{\ast}) > 0$.
...
2.  Accept $\theta^{\ast$ as a sample from $p(\theta | y)$ with probability $p(\theta^* | y) / (M g(\theta^{\ast}))$.
...
-   Conceptually, we retain a sample $\theta^{\ast}$ from $g(\theta)$ by drawing a value 
$u$
from a 
$U(0, M g(\theta^{\ast}))$ distribution and keeping $\theta^{*}$ 
if 
$u \leq p(\theta^{\ast} | y)$.
...
-   Draw $u$ from $U(0, 0.25)$.
    - Let's say $u^* = 0.066$.
    - Since $u^*<q(\theta\mid y) = 0.24$, we accept it as a sample from $q(\theta\mid y)$.
...
-   We draw $u=0.176$ from $U(0, Mg(\theta^*))=U(0, 0.25g(0.185))=U(0, 0.25)$.
