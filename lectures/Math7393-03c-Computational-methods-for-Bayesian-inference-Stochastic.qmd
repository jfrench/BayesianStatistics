...
-   If $p(\theta = \theta^{*} \mid y) > 0$, then $g(\theta = \theta^{*}) > 0$.
...
2.  Accept $\theta^{*}$ as a sample from $p(\theta \mid y)$ with probability $p(\theta^{*} \mid y) / (M g(\theta^{*}))$.
...
-   Conceptually, we retain a sample $\theta^{*}$ from $g(\theta)$ by drawing a value 
$u^{*}$
from a 
$U(0, M g(\theta^{*}))$ distribution and keeping $\theta^{*}$ 
if 
$u^{*} \leq p(\theta^{*} \mid y)$.
...
-   $\text{argmax}_{\theta}q(\theta \mid y) = 0.5$ with $q(0.5 | y) = 0.25.$
-   Pick $M = 0.25$.
