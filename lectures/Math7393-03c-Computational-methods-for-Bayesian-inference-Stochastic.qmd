...
2.  Accept $\theta^{*}$ as a sample from $p(\theta \mid y)$ with probability $\frac{p(\theta^{*} \mid y)}{M g(\theta^{*} )}.$
    -   If $\theta^{*}$ is accepted, continue to step 3.
    -   Otherwise, return to step 1.
3.  Return to step 1 until $B$ draws from $g(\theta)$ have been accepted.

An alternative approach to decide whether we accept the proposed value, $\theta^{*}$,  from $g(\theta)$ is:
