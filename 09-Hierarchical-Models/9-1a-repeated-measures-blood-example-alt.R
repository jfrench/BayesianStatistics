...
parameters {
  real mu;      // common mean
  real<lower=0> sigma_a; // variance of between-subject random effects
  real<lower=0> sigma;  // variance of within-subject random effects
  vector[n] a;  // random effect for mean
}
model {
  // prior distributions
  mu ~ normal(0, sqrt(1000));
  sigma_a ~ student_t(4, 0, 1); //half-t since >= 0
  sigma ~ student_t(4,0, 1);

  // distribution of random effects for mean
  for(i in 1:n) a[i] ~ normal(0, sigma_a);

  // data distribution
  for(i in 1:n) {
    for(j in 1:2) {
      y[i,j] ~ normal(mu + a[i], sigma);
    }
  }
}
...
if (!file.exists("example_9_1_mod_alt.rda")) {
blood_samples = stan(model_code = blood_code,
                     data = blood_data,
                     iter = 10, seed = 23, refresh = 0)
blood_mod = stan_model(model_code = blood_code)
save(blood_mod, file = "example_9_1_mod_alt.rda")
}

load(file = "example_9_1_mod_alt.rda")
blood_samples = sampling(blood_mod,
                         data = blood_data,
                         iter = 5e4, seed = 23)
...
round(post_sum[c("mu", "sigma", "sigma_a", "tvar", "cor"), c("mean", "sd", "2.5%", "97.5%")],
      2)
