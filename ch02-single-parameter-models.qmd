...
- As the sample size increases (with everything else held constant), the data are more influential and our posterior mean converges to $\bar{y}$ and the posterior variance converges to 0.
...
The exponent in the integrand is a quadratic function of $(\tilde{y},\theta)$, so the joint distribution is a bivariate normal distribution. Hence, the marginal distribution of $\tilde{y}$ will be a normal distribution.


Derivation of the posterior predictive mean and variance:
...
The posterior predictive mean is simply the mean of the posterior distribution.

The posterior predictive variance is the sum of the posterior variance and the variance of the data distribution.

Thus,
$$
\tilde{y} \mid y \sim N(\mu_n, \sigma^2 + \tau_n^2)
$$
...
Data distribution: $y_1,y_2,\ldots,y_n\mid\theta\stackrel{\text{i.i.d.}}{\sim}\text{Poisson}(\theta)$, with
$$
p(y_i\mid\theta)=\frac{\theta^{y_i}e^{-\theta}}{y_i!}I_{\{0,1,2,\ldots\}}(y_i)I_{(0,\infty)(\theta)}.
$$
...
$$
for shape parameter $\alpha>0$ and rate (inverse scale) parameter $\beta>0$.
...
$$\tilde{y} \mid y \sim \text{Neg-Bin}(\alpha + \sum_{i=1}^n y_i, \beta + n)$$
...
We set up our prior parameters.
```{r}
alpha <- 3
beta <- 5
```
...
plot(density(post.draws), xlab = "theta",
     main = "100,000 draws from Gamma(6,7)")
...
```{r}
plot(density(post.draws2), xlab = "theta", main = "100,000 draws from Gamma(33,25)")
```
...
$$
i.e., the prior for $\phi$, $\sqrt{J(\phi)}$, can be obtained by directly computing the Fisher information for $\phi$ or finding $\sqrt{J(\theta)}$ and transforming the associated distribution using the change-of-variable formula.

Jeffreys' principle can be extended to multiparameter models.

- Results vary based on whether you multiply the Jeffreys' priors for the parameters independently or use the square root of the determinant of the Fisher information matrix.
...
$$
\hat{\gamma}=\underset{\gamma}{\arg\max}\int p(y\mid\theta)p(\theta\mid\gamma)\,d\theta.
$$
...
If $y_1,\ldots,y_n$ are i.i.d., then the Fisher information for the $n$ observations is $nJ(\theta)$, where $J(\theta)$ is the Fisher information for a single observation.
