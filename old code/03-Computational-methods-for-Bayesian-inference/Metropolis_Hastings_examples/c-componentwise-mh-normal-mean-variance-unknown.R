...
# sigma^2 ~ Inv-Chisq(nu0, sigma0^2) with nu0 = 1 and sigma0 = 0.1.
...
cmh = function(B, start, jump.parm1, jump.parm2) {
  ...
  for (i in 2:(B + 1)) {
    ...
    num.logr = sum(log(dnorm(y, mean = mu.star, sd = sqrt(theta[i - 1, 2])))) +
      ...
      log(dinvchisq(theta[i - 1, 2], df = nu0, scale = sigma0^2)) -
    ...
    num.logr = sum(log(dnorm(y, mean = theta[i, 1], sd = sqrt(sigmasq.star)))) +
      ...
      log(dinvchisq(sigmasq.star, df = nu0, scale = sigma0^2)) -
...
# df.sigmasq = 2
# scale.sigmasq = .1^2
chain1 = cmh(B, start = c(m, s^2), jump.parm1 = .2, jump.parm2 = c(2, .1^2))
chain2 = cmh(B, start = c(1, 0.5^2), jump.parm1 = .2, jump.parm2 = c(2, .1^2))
chain3 = cmh(B, start = c(3, 0.05^2), jump.parm1 = .2, jump.parm2 = c(2, .1^2))
